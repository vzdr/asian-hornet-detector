...
def main():
    output_dir = Path("D:/Ultimate Dataset/wasps_gbif")
    output_dir.mkdir(parents=True, exist_ok=True)

    print("="*70)
    print("GBIF CHUNKED DOWNLOADER - WASPS")
    print("="*70)
    print(f"\nOutput: {output_dir}")
    print(f"Target: 40,000 Wasp images (Vespidae family)")
    print(f"Taxon Key: 4490")
    print()

    media_generator = api.gbif_query_generator(
        taxon_key="4490",  # Vespidae family (GBIF backbone)
    ...
    with open(log_file, 'w') as f:
        f.write(f"GBIF Chunked Download Log - Wasps\n")
        f.write(f"={'='*50}\n\n")
        f.write(f"Download time: {datetime.now()}\n")
        f.write(f"Taxon Key: 52747 (Vespidae family)\n")
