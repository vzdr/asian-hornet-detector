"""
Train EfficientNet-B3 on Balanced GBIF Dataset
No extreme augmentation needed - naturally balanced across all 4 classes

Dataset composition:
- Asian hornets: ~45k (from BeesAndHornets datasets)
- Bees: ~45k (from BeesAndHornets datasets)
- European hornets: ~38k (37k GBIF + 1k archive)
- Wasps: ~31k (GBIF)

Total: ~159k images across 4 balanced classes
"""
...
class GBIFBalancedDataset(Dataset):
    """
    Dataset loading from GBIF downloads + original datasets
    4-class classification: Asian hornets, Bees, European hornets, Wasps
    """
    ...
    def __init__(self, data_sources, split='train', transform=None, samples_per_class=None):
        ...
        if samples_per_class and split == 'train':
            print(f"\nSampling {samples_per_class} images per class for balanced training...")
            for class_name, images in class_images.items():
    ...
    def _load_yolo_data(self, data_sources, split, class_images):
        ...
        for dataset_key in ['bees_hornets1', 'bees_hornets2']:
            ...
            yolo_split = split
            if split == 'val' and not (data_root / 'val').exists():
                yolo_split = 'valid'
            ...
            if not images_dir.exists():
                print(f"  Skipping {dataset_key}/{yolo_split} (images dir not found)")
                continue
            ...
            for label_file in labels_dir.glob('*.txt'):
                ...
                for line in lines:
                    ...
                    # Class 0 = Bee, Class 1 = Asian Hornet (from data.yaml)
                    if class_id == 0:
    ...
    def _load_gbif_european_hornets(self, data_sources, class_images):
        ...
        # Load all image files (excluding .json metadata)
...
def main():
    print("="*70)
    print("TRAINING: 4-Class Balanced GBIF Dataset")
    print("="*70)
    ...
    # Hyperparameters
    BATCH_SIZE = 32  # Optimal for GTX 1650 4GB VRAM
    LEARNING_RATE = 0.001
    ...
    train_dataset = GBIFBalancedDataset(
    ...
    val_dataset = GBIFBalancedDataset(
        data_sources=data_sources,
        split='valid',
    ...
    output_dir = Path('models/gbif_balanced')
    ...
    class_names = ['Asian Hornets', 'Bees', 'European Hornets', 'Wasps']
    print("\nClassification Report:")
