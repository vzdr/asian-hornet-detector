"""
Train EfficientNet-B3 on BALANCED Ultimate Dataset with 30K PER CLASS
Balances all 4 classes to 30,000 samples each with VERY aggressive augmentation for rare classes

Target distribution:
- Asian hornets: 30,000 (use all available)
- Bees: 30,000 (sample from 228k)
- European hornets: 30,000 (1k real × 30 augmented versions)
- Wasps: 30,000 (1k real × 30 augmented versions)

Total: 120,000 perfectly balanced samples (3.3x more than 9k version)
"""
...
class BalancedUltimateDataset(Dataset):
    """
    Balanced dataset with VERY aggressive augmentation for rare classes.
    Binary classification: Asian hornets vs Everything Else

    Strategy:
    - Asian hornets: Use all 36k available
    - Bees: Sample 30k from 228k
    - European hornets: Apply 30x augmentation to reach 30k
    - Wasps: Apply 30x augmentation to reach 30k
    """

    def __init__(self, data_sources, split='train', standard_transform=None,  extreme_transform=None, target_per_class=30000):
    ...
    def _balance_dataset(self, raw_counts):
        ...
        # Asian hornets: Use all available or sample to target
        ...
        # Bees: Sample 30k from available
        ...
        # European hornets: Augment VERY heavily (30x)
        ...
        # Wasps: Augment VERY heavily (30x)
...
def main():
    print("=" * 70)
    print("TRAIN EFFICIENTNET-B3 - BALANCED 30K PER CLASS")
    print("=" * 70)
    print("Strategy: 30,000 samples per class with VERY aggressive augmentation")
    print()
    ...
    TARGET_PER_CLASS = 30000  # 30k per class!
    ...
    # EXTREME augmentation for rare classes (European hornets, wasps) - 30x!
    ...
    # Load datasets
    print("Loading balanced training dataset (30k per class)...")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            }, MODEL_DIR / 'best_balanced_30k_efficientnet.pth')
    ...
    checkpoint = torch.load(MODEL_DIR / 'best_balanced_30k_efficientnet.pth', weights_only=False)
    ...
    results = {
        ...
        'dataset': 'Balanced Ultimate Dataset (30k per class)',
        'target_per_class': TARGET_PER_CLASS
    }

    with open(MODEL_DIR / 'balanced_30k_efficientnet_results.json', 'w') as f:
        json.dump(results, f, indent=2)

    print("\n" + "=" * 70)
    print("RESULTS SAVED")
    print("=" * 70)
    print(f"Best model: {MODEL_DIR / 'best_balanced_30k_efficientnet.pth'}")
    print(f"Training results: {MODEL_DIR / 'balanced_30k_efficientnet_results.json'}")
    print()
