"""
Evaluate Multi-Class Model with Professional Plots
Uses supervisor's plotting functions to generate ROC-AUC, PR curves, and confusion matrices

Run with: py -3.10 evaluate_multiclass_with_plots.py
"""
...
# Output directory
OUTPUT_DIR = Path("multiclass_evaluation_plots")
                    ...
                    with open(label_file) as f:
                        ...
                        if img_path.exists():
                            if yolo_class == 0:  # bee
                                ...
                                self.labels.append(self.class_to_idx['bees'])
                            elif yolo_class == 1:  # asian hornet
                                self.image_paths.append(str(img_path))
                                self.labels.append(self.class_to_idx['asian_hornets'])
                except:
                    continue
        ...
        if 'gbif_european_hornets' in data_sources:
            ...
            if euro_dir.exists():
                for img_path in euro_dir.glob('*.jpg'):
                    self.image_paths.append(str(img_path))
                    self.labels.append(self.class_to_idx['european_hornets'])
        ...
        if 'gbif_wasps' in data_sources:
            ...
            if wasp_dir.exists():
                for img_path in wasp_dir.glob('*.jpg'):
                    self.image_paths.append(str(img_path))
                    self.labels.append(self.class_to_idx['wasps'])
...
def main():
    print("="*70)
    print("MULTI-CLASS MODEL EVALUATION WITH PROFESSIONAL PLOTS")
    print("="*70)
    ...
    val_dataset = MultiClassDataset(data_sources, split='valid', transform=transform)
    ...
    # Raw counts
    cm_file = str(OUTPUT_DIR / 'multiclass_cm.png')
    ...
    cm_norm_file = str(OUTPUT_DIR / 'multiclass_cm_normalize.png')
    plot_confusion_matrix(
        cm,
        classes=CLASS_NAMES,
